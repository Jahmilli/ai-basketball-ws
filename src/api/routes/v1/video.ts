...
export default (app: Router): void => {
  ...
  route.post("/create", validationMiddleware(uploadVideoSchema, "body"), async (req: Request, res: Response) => {
    ...
    try {
      ...
      logger.warn(`An error occurred when writing video ${util.inspect(video)} to database ${formatError(err)}`);
      res.send(500);
    }
  });
  ...
  route.post("/stream", async (req: Request, res: Response) => {
    logger.info(`Received multipart request with body ${util.inspect(req.body)}`);
    ...
    busboy.on("file", (fieldname, file) => {
      ...
      const { writeStream, managedUpload } = s3.upload(s3Config.videosBucket, key);
